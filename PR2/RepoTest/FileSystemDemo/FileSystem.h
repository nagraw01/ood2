...
namespace FileSystem
{
  ...
  {
  public:
	  bool isPackage(std::string filePath);
	  void getDirecLoop(std::string filePath);
	  std::vector<std::string> recurseDirec(std::string filePath);
  private:
  ...
  bool DirecHelper::isPackage(std::string filePath)
  {
	  std::vector<std::string> cppfiles = Directory::getFiles(filePath, "*.cpp");
	  std::vector<std::string> hfiles = Directory::getFiles(filePath, "*.h");
	  if (cppfiles.size() != 0 || hfiles.size() != 0)return true;
	  return false;
  }

  std::vector<std::string> DirecHelper::recurseDirec(std::string filePath)
  {
	  getDirecLoop(filePath);
	  return packageList;
  }

  void DirecHelper::getDirecLoop(std::string filePath)
  {
	  if (isPackage(filePath)) {
		  std::cout << filePath << std::endl;
		  packageList.push_back(filePath);
		  return;
	  }

	  std::vector<std::string> direcs = Directory::getDirectories(filePath);
	  for (size_t i = 2; i < direcs.size(); i++) {
		  getDirecLoop(Path::fileSpec(filePath, direcs[i]));
	  }
  }
}
