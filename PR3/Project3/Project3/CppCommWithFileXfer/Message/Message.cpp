...
int main()
{
  ...
  msg.from(EndPoint("localhost", 8081));msg.command("doIt");
  msg.bodyLength(42);msg.file("someFile");msg.show();
  ...
  newMsg.show();
  SUtils::title("testing assignment");Message srcMsg;
  srcMsg.name("srcMsg");srcMsg.attribute("foobar", "feebar");
  srcMsg.show();std::cout << "\n  assigning srcMsg to msg #1";
  newMsg = srcMsg;newMsg.show();
  std::cout << "\n\n";
  return 0;
}
#endif
